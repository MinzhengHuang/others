...
public class WheelView extends View {
     ...
     */
    private int ADDITIONAL_ITEM_HEIGHT = 30;
    ...
    private void initData(Context context) {
        ADDITIONAL_ITEM_HEIGHT = dip2px(context, 20);
        TEXT_SIZE = dip2px(context, 15);
        ITEM_OFFSET = dip2px(context, 4);
    ...
    public int getCurrentYearValue() {
        return adapter.getMinValue() + getCurrentItem();
    }

    public void setCurrentYearItem(int index) {
        if (adapter == null || adapter.getItemsCount() == 0) {
            return; // throw?
        }
        if (index > 0 && index >= adapter.getMinValue()
                && index <= adapter.getMaxValue()) {
            int nowItem = index - adapter.getMinValue();
            if (nowItem != currentItem) {
                invalidateLayouts();
                int old = currentItem;
                currentItem = nowItem;
                notifyChangingListeners(old, currentItem);
